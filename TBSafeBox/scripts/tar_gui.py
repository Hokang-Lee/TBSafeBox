...
class TarTab(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        # ② メインウィンドウをコンテンツに応じて自動リサイズ
        self.after(50, self._auto_resize_main)
    ...
    def _auto_resize_main(self):
        """② メインウィンドウをコンテンツに応じて自動リサイズ（センタリング付き）"""
        try:
            ...
            MIN_W = int(820 * scale)
            MIN_H = int(680 * scale)
            req_w = max(root.winfo_reqwidth(), MIN_W)
            req_h = max(root.winfo_reqheight(), MIN_H)
            sw = root.winfo_screenwidth()
            sh = root.winfo_screenheight()
            margin = 32
            w = min(req_w, sw - 2 * margin)
            h = min(req_h, sh - 2 * margin)
            x = (sw - w) // 2
            y = (sh - h) // 2
 ...
 # --- generation selector ---
    ...
    def _open_generation_selector(self):
        ...
        def _smart_center(_win, parent=None, min_w=860, min_h=520, margin=24):
        ...
        def build_tree(parent, row_count=14):

            cols = ("name","size","mtime","sha","lst","path")

            # ③ 項目数に応じて高さを自動調整（最小5行・最大20行）
            dynamic_h = max(5, min(row_count + 2, 20))

            tree = ttk.Treeview(parent, columns=cols, show="headings", height=dynamic_h)
        ...
        tree_local = build_tree(frm_local, row_count=len(rows_local) if rows_local else 5)
        ...
        try:
            ...
            if _reachable:
                ...
                _remote_err_msg = f"No archives in {dest}\n(Network unreachable: {_err})"
        ...
        tree_remote = build_tree(frm_remote, row_count=len(rows_remote) if rows_remote else 5)

        try:

            if _remote_err_msg:

                ttk.Label(frm_remote, text=_remote_err_msg).pack(fill="x", padx=8, pady=6)

            elif not rows_remote:
        ...
        _smart_center(win, self.winfo_toplevel(), min_w=860, min_h=520, margin=24)
